...
def get_t_wall_data(
    ...
    if current_direction == 0:
        ...
        if increased_boundary_data:
            # top left boundary
            wall_data = np.vstack((wall_data, gen_data_in_rectangle([0, mid_xy[0] + block_dx/2], 
                                                                    [mid_xy[1] - block_dy - top_boundary_width, mid_xy[1]], 
                                                                    mode=mode, 
            ...
            if mid_xy[1] > block_dy:
                ...
                wall_data = np.vstack((wall_data, gen_data_in_rectangle([mid_xy[0] + block_dx/2, x_max], 
                                                                        [mid_xy[1] - block_dy - top_boundary_width, mid_xy[1] - block_dy], 
                                                                        mode=mode, 
                                                                        density=boundary_density)))
    ...
    if increased_boundary_data:
        ...
        if mid_xy[1] > block_dy: 
            ...
            wall_data = np.vstack(
                (wall_data, gen_data_in_rectangle(
                    [boundary_width, x_max-boundary_width], 
                    [0, block_dy], 
                    mode=mode, 
                    density=boundary_density)))
        else:
            ...
            wall_data = np.vstack(
                (wall_data, gen_data_in_rectangle(
                    [boundary_width, mid_xy[0] + block_dx/2 - boundary_width], 
                    [0, block_dy], 
                    mode=mode, 
                    density=boundary_density)))
